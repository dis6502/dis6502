 ...
 * <p>
 * {@link com.wudsn.tools.dis6502.model.version22.MemoryType} is a frozen
 * snapshot of this enum's ordinals, kept only to guarantee - by throwing at
 * class load time otherwise - that this enum's ordinals (used as raw bytes
 * in a memory inspector's type buffer) never silently drift from the
 * encoding used since application version 2.2.
 *
 * @author Peter Dell
 */
public enum MemoryType {
	...
	/** The values in enum declaration order, for indexing a type buffer by ordinal. */
	 ...
	 * Ensures binary compatibility of the type encoding.
	 */
	...
	/**
	 * The ASCII-to-"internal" (Atari screen code) transform applied when
	 * typing a character into an {@link #SBYTE}-typed byte's cell, used by
	 * {@link MutableMemoryInspectorState#typeEditChar}. This is the WRITE
	 * direction, gated per-byte on its type and only ever given a typed
	 * ASCII character (0-127); it is deliberately kept separate from
	 ...
	 * range but are kept as distinct, differently-shaped formulas rather
	 * than sharing one generalized method.
	 */
